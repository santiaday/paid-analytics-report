#!/usr/bin/env python3
"""
Shared renderer for the Meta Ads WEEKLY and MONTHLY reports.
...
and call render(cfg). All the locked rules — the $1,500 CPD target, the demo
source chain, helpers — live in build.py and are imported here. ONE source of
truth (see CLAUDE.md / spec). Meta differences from the Google/Microsoft copy:
no impression-share columns/chips (Meta has no IS metrics), a CPM column+chip
instead, no Ex-Branded roll-up, and the summary card is a view toggle
(All Campaigns → one view per campaign).
...
import json, os
import build
from build import (g, micros, money, money2, conv, intf, pct, esc, short_name,
                   is_branded, agg_spend, agg_demos, sum_group, sum_view,
                   status_cpd, status_demos, CPD_TARGET, tip,
...
def metric_rows(rows, key_fn):
    """Aggregate campaign/ad-set metric rows by key. (No impression-share metrics
    on Meta; CPM = cost per 1,000 impressions replaces them.)"""
    out = {}
    for r in rows:
        k = key_fn(r)
        cost = micros(g(r, "metrics.cost_micros"))
        o = out.setdefault(k, {"cost": 0.0, "clicks": 0, "imps": 0})
        o["cost"] += cost
        o["clicks"] += int(g(r, "metrics.clicks") or 0)
        o["imps"] += int(g(r, "metrics.impressions") or 0)
    for o in out.values():
        o["cpc"] = avg_cpc(o["cost"], o["clicks"])
        o["cpm"] = (o["cost"] / o["imps"] * 1000) if o["imps"] > 0 else None
    return out
...
# ── Cohort demo override (weekly/monthly) ──────────────────────────────────────
# If the period's routine wrote data/<dir>/cohort_demos.json (Brain ma-funnel
# fb+ig last-touch, or the Salesforce Day-0 fallback), the report swaps Meta
# platform pixel demos (inflated ~2.5-3x by view-through attribution) for the
# cohort numbers. The daily report uses build.py's own template + its own file.
...
def bucket_metrics(rows, idx_of, n_buckets, date_field="segments.date"):
    ...
    for r in rows:
        ...
        if i is None:
            continue
        spend[i] += micros(g(r, "metrics.cost_micros"))
...
def bucket_clicks_data(series_metrics, demo_series, idx_of, n_buckets, labels, date_field="segments.date",
    ...
    for r in series_metrics:
        ...
        c["cost"][i] += micros(g(r, "metrics.cost_micros"))
    ...
    # No Ex-Branded roll-up on Meta (no Branded campaign — it would duplicate All).
    ...
    # No IS metrics on Meta → rankis/imprshare chips never render; CPM rides on cost+imps.
    res = {"labels": labels, "has_demos": has_demos, "has_cost": has_cost,
           "has_rankis": False, "has_imps": has_imps, "has_imprshare": False,
           ...
           "style": {"clicks": "#162050", "demos": "#3185FC", "cpc": "#0D9488", "cpd": "#7C3AED",
                     "cpm": "#0284C7", "imps": "#64748B", "spend": "#EA580C", "ctd": "#DB2777"},
           "campaigns": out}
    if avg:
...
def metric_cells(cur, demos_cur, prior, demos_prior, demos_na=False):
    """Return the 7 metric <td> cells (Spend → Cost/Demo) for one campaign or
    ad set, each with its WoW/MoM delta badge. `cur`/`prior` are metric dicts;
    demos are floats (0 if none). When `demos_na` is True (cohort mode at the
    ad-set level — the cohort has no ad-set split), the Demos and Cost/Demo
    cells render '—' instead of a misleading 0."""
    ...
    cells = [
        ...
        _cell(money2(cur["cpm"]),  money2(P.get("cpm")),  delta_badge(cur["cpm"], P.get("cpm"), lower_is_better=True)),
...
def per_campaign_table(camp_cur, camp_prior, cdem_cur, cdem_prior,
    ...
    for ci, (name, cur) in enumerate(active):
        ...
        # child ad sets for this campaign, active, by spend desc
    ...
    head = ('<tr><th>Campaign / Ad Set</th><th class="num">Spend</th>'
            '<th class="num">Clicks</th><th class="num">Avg CPC</th>'
            '<th class="num">CPM</th><th class="num">Impr.</th>'
...
def render(cfg):
    ...
    _src0 = (cohort.get("data_source", "brain") if cohort else "platform")
    demo_label = {"brain": "Demos (Brain, last-touch)",
                  "salesforce": "Demos (SF Day-0 cohort)"}.get(_src0, "Demos Scheduled")

    # ── summary card: view toggle — All Campaigns + one view per campaign ──────
    sc = agg_spend(camp_cur); sp = agg_spend(camp_prior)
    def summary(pred):
        s, dm, cpd = sum_view(sc, cdem_cur, pred)
        ps, pdm, pcpd = sum_view(sp, cdem_prior, pred)
        return [
    ...
    camp_names = [r["name"] for r in sorted(sc, key=lambda r: -r["cost"])]
    ...
    for vi, (vlabel, vname) in enumerate(views):
        ...
        view_tables += (f'<div class="sumview{on}" id="sumview{vi}">'
                        f'{summary_table(summary(pred), cfg["this_label"], cfg["prior_label"])}</div>')
    ...
    clicks_section = (
        ...
        'toggle the metric · overlay campaigns</span>' +
        tip('<p>First chart: <b>All Campaigns</b> total &mdash; toggle '
            'any campaign chip to overlay it. Then one chart per campaign. Each chart '
            'toggles <b>Demos</b> &middot; <b>Clicks</b> &middot; <b>Impressions</b> &middot; <b>Spend</b> &middot; '
            '<b>CPC</b> &middot; <b>CPM</b> &middot; <b>CPD</b> &middot; <b>Click&rarr;Demo</b>. '
    ...
    per_sanity = [(vl, sum_view(sc, cdem_cur, (lambda nm, _v=vn: nm == _v)))
                  for vl, vn in views[1:]]
    ...
    if cohort:
        ...
        extra = (f"~{conv(unattr_last)} more demos this {ul} matched paid Meta (fb/ig) traffic but not "
    ...
    if basis_src == "salesforce":
        ...
        demo_footnote = (
            ...
            '<code>UTM_Source__c</code> fb/ig), attached to each campaign by <code>Lead.campaign_ID__c</code> = '
            'the Meta <b>campaign ID</b>. ⚠️ SF UTM is <b>first-click</b>, so this basis <b>undercounts '
            'Retargeting ~2&times;</b> (retargeting is rarely the first touch) and runs ~3&ndash;13% below the '
            'Brain total — a documented attribution difference, not a drop. <b>Ad-set rows show '
            '&ldquo;&mdash;&rdquo; for Demos / Cost-Per-Demo</b> &mdash; the cohort has no ad-set split. '
            f'{extra}<b>Prior</b> = the {ul} immediately before. CPD target &le; ${CPD_TARGET:,}.</p>'
        )
    elif basis_src == "brain":
        basis_note = '<span class="pill pill-cohort">Demos: Brain (fb+ig, last-touch)</span>'
        demo_footnote = (
            ...
            'campaign via the <code>current_campaign</code> filter. Last-touch gives Retargeting proper credit '
            '(Salesforce first-click UTM undercounts it ~2&times;), and the session basis never matures — '
            f'apples-to-apples {ul}-over-{ul}, unlike Meta platform pixel conversions (which run ~2.5-3&times; '
            'high on view-through attribution). <b>Ad-set rows show &ldquo;&mdash;&rdquo; for Demos / '
            'Cost-Per-Demo</b> &mdash; the Brain funnel has no ad-set split. '
            f'{extra}<b>Prior</b> = the {ul} immediately before. CPD target &le; ${CPD_TARGET:,}.</p>'
        )
    else:
        basis_note = ('<span class="pill pill-platform">Demos: Meta platform conversions '
                      '(Brain + Salesforce unavailable)</span>')
        demo_footnote = (
            '<p>&dagger; <b>Demos</b> = the Meta pixel custom conversion <i>&ldquo;Demo Scheduled - URL '
            'Traffic (Thanks)&rdquo;</i>. ⚠️ Meta&rsquo;s attribution windows (7-day click + 1-day view) make '
            'this run <b>~2.5&ndash;3&times; ABOVE</b> the Salesforce demo count — treat it as a relative '
            f'trend, not an absolute count. <b>Prior</b> = the {ul} immediately before. <i>(Last-resort '
            'basis: both the Brain and the Salesforce fallback were unavailable this run.)</i> '
    ...
    perc_tip = tip(f'<p>Δ badge compares this {unit.lower()} vs the prior {unit.lower()}. Green = good, '
                   ...
                   'Sorted by spend. Ad sets indented.</p>')
    ...
    html = PAGE.format(
        ...
        view_chips=view_chips, view_tables=view_tables, perc_table=perc_tbl,
        perc_tip=perc_tip,
    ...
    for vl, (s_, d_, c_) in per_sanity:
        print(f"  This {unit} — {vl}: spend={money(s_)} demos={conv(d_)} cpd={money(c_)}")
    print(f"  Active campaigns: {n_active} | ad sets (cur, imps>0): "
...
<body><div class="wrap">
  <header>
    <h1>{headline} <span class="pill">Facebook + Instagram</span>{basis_note}</h1>
  ...
  <div class="card accordion">
    <h2>Per-Campaign Performance <span class="dt">— {n_active} active campaigns · click <b>+</b> to expand ad sets · value + {unit}-over-{unit} Δ</span>{perc_tip}<span class="chev">▼</span></h2>
    <div class="acc-body">
      {perc_table}
    </div>
  </div>
...
(function(){{
  ...
  // Performance Summary view toggle
