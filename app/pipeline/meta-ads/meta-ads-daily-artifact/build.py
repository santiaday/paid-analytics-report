#!/usr/bin/env python3
"""
Meta Ads Daily Snapshot — HTML builder.

Reads the Meta Marketing API results saved under data/<ANCHOR_DATE>/ and
renders a single self-contained HTML file to outputs/<ANCHOR_DATE>.html.

Usage:
    python3 build.py 2026-06-10
...
Meta Graph API and writes the JSON files (in Google-style dotted-key shape,
e.g. "campaign.name"; "ad_group.name" = the Meta AD SET name); this script only
computes + renders. That data shim is why this renderer is a near-verbatim port
of the Google/Microsoft daily builders. Meta differences (see CLAUDE.md):
  • NO search terms / bad-terms audit / keyword vocabulary (Meta has no search queries)
  • NO impression-share metrics (rank-lost / budget-lost / impr share don't exist on Meta)
  • NO recommendations section
  • The summary card is ONE table with a view toggle: All Campaigns → one view per
    campaign (Prospecting first, then Retargeting — ordered by spend)
  • Charts gain a CPM toggle (cost ÷ impressions × 1000)
"""
...
# ── Config ────────────────────────────────────────────────────────────────
CPD_TARGET   = 1500          # Meta Cost-Per-Demo monitoring target ($) — set by David
                             # 2026-06-11 (Meta cohort CPD runs ~$1,400; Google's north
                             # star is $2,100 but Meta demos are structurally cheaper).
...
# Platform demo definition note: the Meta demo conversion ("Demo Scheduled - URL
# Traffic (Thanks)", custom conversion REDACTED) is filtered AT FETCH TIME in
# fetch_data.py (DEMO_ACTIONS) — this renderer just reads demos_cur/prior/30d. The
# platform basis is the LAST RESORT only: it runs ~2.5-3x ABOVE the Salesforce demo
# count (Meta's 7d-click + 1d-view attribution windows). Normal basis = Brain, then
# Salesforce (see the demo-source pill / CLAUDE.md "Cohort demo override").
...
# Meta has no "Branded" campaign (its split is Prospecting vs Retargeting), so
# is_branded() is always False here. KEPT because report_common.py (the shared
# weekly/monthly renderer, a byte-copy of Google's) imports it; with no match the
# Ex-Branded roll-up simply never differs from All Campaigns.
def is_branded(name): return bool(re.search(r"\bbrand", name or "", re.I))
...
def load_cohort_demos(anchor):
    """Daily Brain/Salesforce cohort override, if the routine wrote data/<anchor>/cohort_demos.json
    (current_source fb+ig, via cohort_demos.py daily). Absent → platform pixel demos (the daily
    last resort). load() returns [] when missing (falsy) and the parsed dict when present (truthy)."""
...
def load_pipeline(anchor):
    ...
    data/<anchor>/pipeline_30d.json (Salesforce, via pipeline_sf.js). Absent → None →
    the grid simply omits the opps/accounts/ARR tiles."""
...
def sum_view(spend_rows, demos_map, pred):
    """Spend/demos/CPD for the campaigns matching pred(name) — powers the summary
    card's view toggle (All Campaigns → one view per campaign)."""
    spend = sum(r["cost"] for r in spend_rows if pred(r["name"]))
    demos = sum(d for n, d in demos_map.items() if pred(n))
    cpd = (spend / demos) if demos > 0 else None
    return spend, demos, cpd
...
def alert_entities(data):
    ...
    Account roll-ups + campaigns come from the All-Campaigns chart entry; ad sets
    (kind 'adgroup' — the internal key is kept for parity with the Google engine)
    from the per-campaign entries (their 'All ad sets' total is skipped — it
    duplicates the campaign). Ad sets are spend/clicks/imps/CPC only (demos too
    sparse — by design)."""
    ents = []
...
def render_alerts_section(alerts, available, cfg=ALERTS_CFG):
    ...
    if not alerts:
        return ('<p class="ok-line">✅ No material changes vs the 28-day baseline '
                '(campaigns, ad sets &amp; account roll-ups).</p>', 0)
    KIND = {"account": "Account", "campaign": "Campaign", "adgroup": "Ad set"}
    rows = ""
...
def clicks_daily_data(rows, anchor_date, n_days=CLICKS_DAYS, demos_rows=None,
    ...
    # "All Campaigns" full-width chart — combined total + per-campaign. (No Ex-Branded
    # line on Meta — there is no Branded campaign; the per-campaign chips cover the split.)
    ...
    for ci, camp in enumerate(corder):
        ...
        N = max(len(agorder), 1)
        dsets = [{"label": "All ad sets", "color": "#3185FC", "total": True,
    ...
    style = {"clicks": "#162050" if daily else "#3185FC", "demos": "#3185FC",
             "cpc": "#0D9488", "cpd": "#7C3AED", "rankis": "#B45309", "cpm": "#0284C7",
    ...
    if daily:
        ...
        # View toggle (daily-only): Current view vs Metric Grid
        result["views"] = True
    return result
...
def clicks_section_html(title_suffix):
    ...
    return (
        '<div class="card">'
        f'<h2>Daily Metrics <span class="dt">— {esc(title_suffix)}</span>' +
        tip('<p>First chart: <b>All Campaigns</b> total &mdash; toggle any '
            'campaign chip to overlay it. Remaining charts: one per campaign with <b>ad-set</b> chips '
            '(the Meta analog of ad groups). Each chart toggles '
            '<b>Demos</b> (default) &middot; <b>Clicks</b> &middot; <b>Impressions</b> &middot; '
            '<b>Spend</b> (total cost, $) &middot; '
            '<b>CPC</b> (avg cost per click, $) &middot; <b>CPM</b> (cost per 1,000 impressions, $) &middot; '
            '<b>CPD</b> (cost per demo, $) &middot; <b>Click&rarr;Demo</b> (conversion rate, %). '
            'Default shows totals only. The <b>&#12316; 7-day avg</b> chip overlays a dashed trend line '
            '(ratio metrics use rolling 7-day totals, so low-volume days don\'t distort it).</p>') +
        '</h2>'
...
(function(){
  ...
  var ST=D.style||{clicks:'#3185FC',demos:'#3185FC',cpc:'#0D9488',cpd:'#7C3AED',cpm:'#0284C7',rankis:'#B45309',imps:'#64748B',imprshare:'#15803D',spend:'#EA580C',ctd:'#DB2777'};
  ...
  var MLBL={demos:{t:'Demos',h:'Demos scheduled per day'},
            ...
            cpm:{t:'CPM',h:'Cost per 1,000 impressions per day'},
            ...
            opps:{t:'Opportunities',h:'Opportunities created per day — booked-on-day (Opportunity Created date), from Salesforce (booked-on-day; first-click fb/ig attribution)'},
            accts:{t:'Accounts',h:'New customer accounts per day — booked-on-day (Stripe first-invoice date), from Salesforce (booked-on-day; first-click fb/ig attribution)'},
            arr:{t:'ARR',h:'New ARR booked per day (after coupons) — booked-on-day, from Salesforce (booked-on-day; first-click fb/ig attribution)'}};
  var MWORD={clicks:'clicks',demos:'demos',imps:'impressions',spend:'',cpc:'',cpd:'',cpm:'',rankis:'',imprshare:'',ctd:'',opps:'opps',accts:'accounts',arr:''};
  ...
  function fmt(v,m){
    ...
    if(m==='cpm')    return '$'+(Math.round(v*100)/100).toLocaleString();
  ...
  (D.campaigns||[]).forEach(function(camp){
    ...
    if(HAS_COST && HAS_IMPS) modes.push('cpm');   // cost per 1,000 impressions (Meta-only)
    ...
    function cpmArr(e){   // cost per 1,000 impressions per day (null on 0-imp days → gap)
      var a=[]; if(!e.imps) return a;
      for(var i=0;i<e.imps.length;i++){ var n=e.imps[i]; a.push(n>0?(e.cost[i]/n*1000):null); } return a;
    }
    ...
    function maFor(e,m){  // MA series for entity e in mode m (null → no MA for this line)
      ...
      if(m==='cpm'){ if(!e.imps) return null;
        return rsum(e.cost,e.imps,N).map(function(v){return v==null?null:v*1000;}); }
    ...
    function datasetsFor(m){
      ...
      ents.forEach(function(e){
        ...
        else if(m==='cpm'){ out.push(mkLine(e,cpmArr(e),'y','cpm',false,'')); addMA(out,e,m); }
    ...
    function scalesFor(m){
      ...
      if(m==='both'){
        ...
        s.y={beginAtZero:true,grid:{color:gridc},ticks:{callback:function(v){return '$'+v;}},title:{display:true,text:'CPM',font:{size:9}}};
      } else if(m==='arr'){
  ...
  // The first chart's datasets (All campaigns / per-campaign) are the selectable
  ...
  function gridModes(e){
    ...
    if(HAS_COST&&HAS_IMPS) ms.push('cpm');
  ...
  function gridArr(e,m){
    ...
    if(m==='cpm'){ if(!e.imps) return null; a=[]; for(i=0;i<e.imps.length;i++){ a.push(e.imps[i]>0?e.cost[i]/e.imps[i]*1000:null); } return a; }
  ...
  function gridScale(m){
    ...
    else if(m==='cpc'||m==='cpd'||m==='cpm'){ s.y={beginAtZero:true,grid:{color:gridc},ticks:{callback:function(v){return '$'+v;}}}; }
...
# demos_cur/prior are OPTIONAL on Meta: a genuine zero-demo day writes an empty list,
# and when the cohort override is present (the normal case) they aren't used at all.
PREFLIGHT_OPTIONAL = [
...
def render(anchor):
    ...
    # Daily cohort override (Brain → Salesforce; platform pixel fallback when absent).
    ...
    def summary_rows(pred, with_proj):
        s, d, cpd = sum_view(spend_cur, demos_cur, pred)
        ps, pd, pcpd = sum_view(spend_prior, demos_prior, pred)
        rows = [
        ...
        if with_proj:
            rows.append(("Projected monthly spend", money(proj), "—", "—", ""))
        return rows, (s, d, cpd)

    # Summary card views: All Campaigns first, then one view per campaign that spent
    # on the report day, ordered by spend (Prospecting before Retargeting today).
    # Generic on purpose — a future third campaign automatically gets its own view.
    camp_names = [r["name"] for r in sorted(spend_cur, key=lambda r: -r["cost"])]
    views = [("All Campaigns", None)] + [(short_name(n) or n, n) for n in camp_names]
    view_chips, view_tables, sanity = "", "", []
    for vi, (vlabel, vname) in enumerate(views):
        pred = (lambda nm: True) if vname is None else (lambda nm, _v=vname: nm == _v)
        rows, totals = summary_rows(pred, with_proj=(vname is None))
        on = " on" if vi == 0 else ""
        view_chips += f'<span class="vbtn{on}" data-v="{vi}">{esc(vlabel)}</span>'
        view_tables += f'<div class="sumview{on}" id="sumview{vi}">{summary_table(f"tblView{vi}", rows)}</div>'
        sanity.append((vlabel, totals))

    # Clicks & Demos per day (last 30 days, all ad sets). Optional: only if its data
    # file was fetched. demos_30d.json supplements it (platform basis); the cohort
    # override replaces campaign-level demos when present.
    clicks_rows = load(anchor, "clicks_30d.json")
    chart_data = None
    if clicks_rows:
        demos_rows = load(anchor, "demos_30d.json")     # [] if the fetch didn't write it
        pipeline = load_pipeline(anchor)                # booked-on-day opps/accounts/ARR (Salesforce)
        ...
        clicks_section = clicks_section_html(f"last {CLICKS_DAYS} days · all ad sets")
        clicks_js = CLICKS_JS
    else:
    ...
    if basis_src == "salesforce":
        ...
        demo_footnote = (
            ...
            'date (David&rsquo;s CDF1 formula, <code>Original_Demo_Scheduled_At__c</code> basis, '
            '<code>UTM_Source__c</code> fb/ig), attached to each campaign by <code>Lead.campaign_ID__c</code> = the '
            'Meta <b>campaign ID</b>. ⚠️ SF UTM is <b>first-click</b>, so this basis <b>undercounts Retargeting '
            '~2&times;</b> (retargeting is rarely the first touch) and runs ~10&ndash;15% below the Brain total — '
            'a documented attribution difference, not a drop. <b>Prior</b> = the same weekday exactly 7 days '
            'earlier. Chart ad-set demo lines are flat &mdash; the cohort has no ad-set split.</p>')
    elif basis_src == "brain":
        basis_note = ('<span class="pill" style="background:#3185FC;color:#fff;border:0">'
                      'Demos: Brain (fb+ig, last-touch)</span>')
        demo_footnote = (
            '<p>&dagger; <b>Demos = DoorLoop Brain marketing-attribution funnel</b> '
            '(<code>ma-funnel-daily</code> &middot; <code>current_source = fb, ig</code> &middot; '
            '<b>last-touch</b> session attribution &middot; <code>DEMO_SCHEDULED_TOTAL</code> = PostHog '
            'in-session demos + SFDC same-day-MQL&rsquo;d demos), bucketed by session date and split by '
            'campaign via the <code>current_campaign</code> filter. Last-touch gives Retargeting proper '
            'credit (Salesforce first-click UTM undercounts it ~2&times;). <b>Prior</b> = the same weekday '
            'exactly 7 days earlier. Chart ad-set demo lines are flat &mdash; the Brain funnel has no '
            'ad-set split. Falls back to the Salesforce Day-0 cohort, then Meta platform conversions, '
            'if the Brain is unreachable.</p>')
    else:
        basis_note = ('<span class="pill" style="background:#64748B;color:#fff;border:0">'
                      'Demos: Meta platform conversions (Brain + Salesforce unavailable)</span>')
        demo_footnote = (
            '<p>&dagger; <b>Demos Scheduled</b> = the Meta pixel custom conversion <i>&ldquo;Demo Scheduled - '
            'URL Traffic (Thanks)&rdquo;</i>. ⚠️ Meta&rsquo;s attribution windows (7-day click + 1-day view) '
            'make this run <b>~2.5&ndash;3&times; ABOVE</b> the Salesforce demo count — treat it as a relative '
            'trend, not an absolute count. <b>Prior</b> = the same weekday exactly 7 days earlier. '
            '<i>(Platform demos — last-resort basis: both the Brain and the Salesforce fallback were '
            'unavailable this run.)</i></p>')
    ...
    html = TEMPLATE.format(
        ...
        view_chips=view_chips, view_tables=view_tables,
    ...
    for vlabel, (s, d, cpd) in sanity:
        print(f"  {vlabel}: spend={money(s)} demos={conv(d)} cpd={money(cpd)}")
    print(f"  Projected month: {money(proj)} | Demo basis: {basis_src}")
...
<title>Meta Ads Daily Snapshot — {anchor}</title>
<style>
  ...
  *{{box-sizing:border-box}}
  ...
  /* Performance Summary view toggle (All / Prospecting / Retargeting) */
  .view-toggle{{display:inline-flex;gap:0;margin:0 0 12px;border:1px solid var(--border);border-radius:9px;overflow:hidden}}
  .vbtn{{cursor:pointer;font-size:12.5px;font-weight:600;padding:6px 16px;color:var(--text-secondary);
    background:var(--bg-card);user-select:none;border-right:1px solid var(--border)}}
  ...
  .tip{{position:relative;display:inline-flex;align-items:center;justify-content:center;width:15px;height:15px;
    border-radius:50%;background:var(--gray-300);color:var(--text-primary);font-size:10px;font-weight:700;
    cursor:default;margin-left:5px;vertical-align:middle;flex-shrink:0}}
  .tip .tipbox{{display:none;position:absolute;z-index:60;top:150%;left:0;width:380px;max-width:80vw;
    ...
    padding:10px 13px;font-size:12px;font-weight:400;color:var(--text-secondary);line-height:1.5;white-space:normal}}
...
<body><div class="wrap">
  <header>
    <h1>Meta Ads Daily Snapshot <span class="pill">Facebook + Instagram</span>{basis_note}</h1>
