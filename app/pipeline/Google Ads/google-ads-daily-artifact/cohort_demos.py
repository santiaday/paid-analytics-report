#!/usr/bin/env python3
"""
Build the Brain Day-0 cohort demo override for the WEEKLY or MONTHLY report
(+ the weekly exec deck).

Reads a raw `gtm-paid-media-daily-cohort` pull from DoorLoop's Brain MCP and
aggregates DEMO_SCHEDULED_AT_DAY0 into the report's 12-bucket grid, keyed by
Google Ads CAMPAIGN_ID -> report campaign name (so demos line up with spend).
Writes data/<period>-<key>/cohort_demos.json, which report_common.render() and
build_exec_deck.compute() auto-detect to switch demos/CPD/the demo trend to
Brain Day-0 cohort (apples-to-apples, never matures). See
/opt/mcp/Master Context/Brain Connector/MEMORY.md for the Brain pull procedure and why we map by
CAMPAIGN_ID and use Day-0.

Usage:
    # 1) In the MAIN session (never a subagent — the JSON is large), pull the
    #    window the report needs:
...
SOURCE = "google-ads"  # the Microsoft copy of this script sets this to "bing"
...
def main_daily(raw_file, anchor_str, root="."):
    ...
    # GUARD + AUTO-FALLBACK: the Brain cube must actually contain the ANCHOR day. The Brain 24h
    # cache (and the upstream Snowflake load) often lags, so a morning pull can return a cube that
    # ENDS at ANCHOR-1 with no ANCHOR row — silently summing that gives a false "0 demos" for the
    # anchor (shipped 2026-06-21; real Day-0 was 6). A TRUE zero day still has the anchor ROW present
    # (other sources/campaigns logged that date), so "no row for the date anywhere" = missing data,
    # not a real zero. SOURCE CHAIN on a missing anchor: the agent should already have re-pulled the
    # Brain with refresh=true (that retry is MCP-only, so it lives in the routine prompt, not here).
    # If we still arrive with a Brain file that lacks the anchor, AUTO-FALL-BACK IN CODE to the
    # real-time Salesforce Day-0 cohort (sf_cohort.js, read-only) — SF is the source-of-truth and
    # always has yesterday. If SF also fails, exit without writing so build.py's own absent-file
    # fallback to platform conversions (labeled gray pill) takes over. Only enforced for
    # origin=="brain"; the SF fallback file is per-anchor + real-time, so an absent anchor there
    # genuinely means a real zero and is written as-is.
...
def main(raw_file, period, key, root="."):
    ...
    if not rep_ids:
        sys.exit("ERROR: %s/camp_cur.json has no campaign.id values — cohort mapping needs "
                 "campaign ids. (Fetch must include the campaign id.)" % data_dir)
    ...
    for n in sorted(out["campaign_cur"], key=lambda k: -out["campaign_cur"][k]):
        print("    %-30s %6.1f" % (n, out["campaign_cur"][n]))
    print("    %-30s %6.1f   (+%.1f unattributed)" % ("TOTAL", tot, unattributed[-1]))
