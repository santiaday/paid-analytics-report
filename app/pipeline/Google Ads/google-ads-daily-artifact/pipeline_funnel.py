...
google-ads only; campaign attribution is by CAMPAIGN_ID = the Google Ads campaign id (same join
...
def build(raw_path, anchor, source="brain"):
    ...
    for r in rows:
        if (r.get("SOURCE") or "").lower() not in ("google-ads", ""):   # google-ads only (HARD rule #5)
            continue
